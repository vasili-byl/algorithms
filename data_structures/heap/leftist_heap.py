...
class LeftistHeap(object):
    ...
    def __init__(self, head=None, skew_heap=False):
        self.head = head
        self.skew_heap = skew_heap
    ...
    def __meld(self, other):
        if self.head is None and other.head is None:
            ...
            root = self.right_descent(self.head, other.head)
            self.head = root

    def right_descent(self, node1, node2):
        if node1 is None:
            return node2
        if node2 is None:
            return node1
